...
export class SaucesService {
  ...
  getSauces() {
    this.http.get<Sauce[]>('https://projet6.onrender.com/api/sauces').pipe(
  ...
  likeSauce(id: string, like: boolean) {
    return this.http.post<{ message: string }>(
      'https://projet6.onrender.com/api/sauces/' + id + '/like',
  ...
  createSauce(sauce: Sauce, image: File) {
    ...
    return this.http.post<{ message: string }>('https://projet6.onrender.com/api/sauces', formData).pipe(
      catchError(error => throwError(error.error.message))
    );
  }
  ...
  modifySauce(id: string, sauce: Sauce, image: string | File) {
    if (typeof image === 'string') {
      return this.http.put<{ message: string }>('https://projet6.onrender.com/api/sauces/' + id, sauce).pipe(
        catchError(error => throwError(error.error.message))
      );
    } else {
      const formData = new FormData();
      formData.append('sauce', JSON.stringify(sauce));
      formData.append('image', image);
      return this.http.put<{ message: string }>('https://projet6.onrender.com/api/sauces/' + id, formData).pipe(
        catchError(error => throwError(error.error.message))
      );
    }
  }
  ...
  deleteSauce(id: string) {
    return this.http.delete<{ message: string }>('https://projet6.onrender.com/api/sauces/' + id).pipe(
      catchError(error => throwError(error.error.message))
    );
  }
}
